...
with open(blastfile) as fin:
    for line in fin: # .readlines() is the default iter method for the open file class
        ...
        if pcnt > 30 and matchlen > 0.9*qlen:
            ...
            # We won't want to modify the elements so a tuple is "safer"
            # in that we then can't modify it by mistake
...
with open(bedfile) as fin:
    for line in fin:
        # an asterisk before a variable name when unpacking
        # makes that variable store remaining elements
        bed_sid, bed_start, bed_end, gene, *_ = line.split()
...
for blast_sid, blast_sstart, blast_send in hits: # unpack our blast data
    for bed_sid, bed_start, bed_end, gene in feats:
        ...
        # Once we are dealing with features at higher index locations than our hit,
        # go to the next hit (break loop over feats)
        ...
        if (blast_sstart > bed_start
            ...
            # Each BLAST hit will only be in one feature 
            # so move to next hit once you've found it
            break 
